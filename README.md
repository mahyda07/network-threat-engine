...
A real-time network packet analyzer and threat detection system built in C++17 as part of the Tynovate Studio 2026 Internship — Track A.
...
This engine captures network traffic, breaks down every packet layer by layer, groups packets into conversations (flows), detects suspicious patterns, and logs alerts to a database — all in real time with a live terminal dashboard.

In simple terms: it watches your network and raises an alarm when something looks like an attack.
...
- Live ncurses terminal dashboard showing packet count, flows, recent packets, active flows, and alerts
...
```json
{
  "alerts": [
    ...
    },
    {
      ...
      "type": "SYN FLOOD",
      "ip": "127.0.0.1",
      "detail": "20 SYNs with 0 completed handshakes",
      "timestamp": "2026-07-13 23:04:51"
    }
  ]
}
...
```bash
git clone https://github.com/mahyda07/network-threat-engine.git
...
```bash
sudo apt install -y build-essential cmake libpcap-dev libsqlite3-dev libncurses-dev
```
...
**WSL2 Live Capture:** WSL2 does not support raw socket access to the host Windows network interface. Live capture via `pcap_open_live()` is not possible in this environment. All testing uses pcap files generated with `tcpdump` on the loopback interface — the code is identical for both modes, only the open call changes.
